// composables/useExperienceLevels.ts
export function useExperienceLevels() {
  ...
  const getExperienceLevelColor = (score: number): string => {
    const mappedScore = mapToNearestLevel(score)
    
    switch (mappedScore) {
      case 20:
        return 'bg-gray-400' // Básico
      case 40:
        return 'bg-blue-400' // Intermedio
      case 60:
        return 'bg-yellow-400' // Avanzado
      case 80:
        return 'bg-orange-400' // Especialista
      case 100:
        return 'bg-green-500' // Experto
      default:
        return 'bg-gray-400'
    }
  }

  // Get text color class based on experience level
  const getExperienceLevelTextColor = (score: number): string => {
    const mappedScore = mapToNearestLevel(score)
    
    switch (mappedScore) {
      case 20:
        return 'text-gray-700' // Básico
      case 40:
        return 'text-blue-700' // Intermedio
      case 60:
        return 'text-yellow-700' // Avanzado
      case 80:
        return 'text-orange-700' // Especialista
      case 100:
        return 'text-green-700' // Experto
      default:
